...
def vgg10_unet_origin(input_shape=(256,256,3), weights='imagenet'):
    ...
    block6_up = UpSampling2D(size=(2, 2))(block5_drop)
    ...
    block7_up = UpSampling2D(size=(2, 2))(block6_conv3)
    ...
    block8_up = UpSampling2D(size=(2, 2))(block7_conv3)
    ...
    block9_up = UpSampling2D(size=(2, 2))(block8_conv2)
...
if __name__ == '__main__':
    is_train = False
    if is_train:
        model = vgg10_unet_origin(input_shape=(64,64,3), weights='imagenet')

        for layer in model.layers:
            layer.trainable = True
        model.compile(optimizer=Adam(lr=1e-4), loss='binary_crossentropy', metrics=['accuracy'])
        model_checkpoint = ModelCheckpoint('unet.h5', monitor='loss', verbose=1, save_best_only=True)
        model.fit_generator(train_generator(batch_size=1),
                            steps_per_epoch=200,
                            epochs=50,
        ...
        model = vgg10_unet(input_shape=(64,64,3))

        model.load_weights('unet.h5')
        # x = cv2.imread('./data/test/images/imgs/%d.png' % 1)
